class SiteManagerTantabus extends SiteManager
{
    constructor(sitesManager, pageLimit)
    {
		super(sitesManager, SITE_TANTABUS, 'https://tantabus.ai', pageLimit);
    }
    ...
    buildRequestUrl(searchText, pageNumber)
	{
		...
		var possibleAddedKey = this.sitesManager.model.tantabusApiKey ? '&key=' + this.sitesManager.model.tantabusApiKey : '';
		
		return this.url + '/api/v1/json/search/images?q=' + this.prepareQueryForTantabus(query) + '&page=' + pageNumber + '&per_page=' + this.pageLimit + possibleAddedKey;
	}

	prepareQueryForTantabus(searchQuery)
    {
